classdef takagi_function_sim
    properties
        n               % number of train kereta
    ...
    methods
        ...
        function dmin = tkg_distance(b,j)
            ...
            dmin = b.dSafe + dBB;               % ...(1) dmin = dSafe + dBB
        end
        
        % Synchronisation Control of Trains
        function bbeta = tkg_sync(b,j)
            if j==1
                lastSpeed=b.v(b.n);
            else, lastSpeed=b.v(j-1);
            end
            bbeta = b.aMax*(b.v(j)-lastSpeed)/b.v(j);
        ...
        function train = trn_ahead(b,j)
            ...
            for h = 1:b.n
                ...
                if h~=j && ( nose2tail <= tkg_distance(b,j) ...
                        || ( b.strt_ctrl(j) == 1 && b.v(h) >= b.vMax - b.aMax ) ) ...
                        && sta_near(b,j) > nose2tail
                     train = 1;
                end
            end
        end
